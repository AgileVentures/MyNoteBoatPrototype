...
export default class TestNavLightsScreen extends React.Component {
  static navigationOptions = {
    title: 'Tester les feux de navigation',
  };
  ...
  componentDidMount() {
    AsyncStorage.getItem('@MyNoteBoatStore:TestNavLights').then((value) => {
      if (value === null){ value = "{}" }
      this.setState({
        isLoading: false,
  ...
  async loadStoredData() {
    var value = "{}"
    try {
      value = await AsyncStorage.getItem('@MyNoteBoatStore:TestNavLights');
      if (value !== null){
  ...
  async onPress() {
    ...
    if (value) { // if validation fails, value will be null
      ...
      try {
        await AsyncStorage.setItem('@MyNoteBoatStore:TestNavLights', JSON.stringify(value));
      } catch (error) {
