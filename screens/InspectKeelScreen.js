...
export default class InspectKeelScreen extends React.Component {
  ...
  componentDidMount() {
    AsyncStorage.getItem('@MyNoteBoatStore:InspectKeel').then((value) => {
      if (value === null){ value = "{}" }
      this.setState({
        isLoading: false,
  ...
  async loadStoredData() {
    var value = "{}"
    try {
      value = await AsyncStorage.getItem('@MyNoteBoatStore:InspectKeel');
      if (value !== null){
  ...
  async onPress() {
    ...
    if (value) { // if validation fails, value will be null
      ...
      try {
        await AsyncStorage.setItem('@MyNoteBoatStore:InspectKeel', JSON.stringify(value));
      } catch (error) {
