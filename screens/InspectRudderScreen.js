...
export default class InspectRudderScreen extends React.Component {
  ...
  componentDidMount() {
    AsyncStorage.getItem('@MyNoteBoatStore:InspectRudder').then((value) => {
      if (value === null){ value = "{}" }
      this.setState({
        isLoading: false,
  ...
  async loadStoredData() {
    var value = "{}"
    try {
      value = await AsyncStorage.getItem('@MyNoteBoatStore:InspectRudder');
      if (value !== null){
  ...
  async onPress() {
    ...
    if (value) { // if validation fails, value will be null
      ...
      try {
        await AsyncStorage.setItem('@MyNoteBoatStore:InspectRudder', JSON.stringify(value));
      } catch (error) {
